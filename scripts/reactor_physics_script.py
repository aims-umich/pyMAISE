...
# Read data
data = pd.read_csv(
    "https://raw.githubusercontent.com/aims-umich/pyMAISE/develop/pyMAISE/data/xs.csv"
)
X = data.iloc[:, :-1]
print(X)
Y = data.iloc[:, [-1]]
...
scores = score_model(scores, best_models, xtrain, ytrain[:, 0], "Train")
scores = score_model(scores, best_models, xtest, ytest[:, 0], "Test")

scores = pd.DataFrame(scores)
print(scores)
scores.to_csv("reactor_physics_testing_metrics.csv", index=False)
