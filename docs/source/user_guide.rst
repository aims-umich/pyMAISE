...
After creating a model, an explainability analysis can be performed with features in the :class:`pyMAISE.explain.ShapExplainers`. A sample explainability analysis with SHAP is shown in the CHF benchmark. These include

- :meth:`pyMAISE.explain.ShapExplainers.DeepLIFT`: fits a DeepLIFT explainer to evaluate SHAP coefficients,
- :meth:`pyMAISE.explain.ShapExplainers.IntGradients`: fits an Integrated Gradient explainer to evaluate SHAP coefficients,
- :meth:`pyMAISE.explain.ShapExplainers.KernelSHAP`: fits a KernelSHAP explainer to evaluate SHAP coefficients,
- :meth:`pyMAISE.explain.ShapExplainers.Exact_SHAP`: fits an Exact SHAP explainer to evaluate SHAP coefficients,
- :meth:`pyMAISE.explain.ShapExplainers.postprocess_results`: generates SHAP mean values for plotting functions,
- :meth:`pyMAISE.explain.ShapExplainers.plot`: makes a beeswarm plot and a bar plot for each SHAP method or for a particular method, and
- :meth:`pyMAISE.explain.ShapExplainers.plot_bar_only`: makes a bar plot for each or a particular SHAP method.
